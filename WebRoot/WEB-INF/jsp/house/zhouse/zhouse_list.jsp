			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="zhouse/list.do" method="post" name="Form" id="Form">
...
<c:if test="${userid == var.USERID }">
	<input type='checkbox' name='ids' value="${var.ZHOUSE_ID}" class="ace" />
</c:if>
...
<span class="lbl"></span></label>
											...
											<td class='center'>${var.HSGJ}</td>
											<td class='center'>${var.FLOOR}</td>
											...
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													<c:if test="${userid == var.USERID }">
													<a class="btn btn-xs btn-success" title="编辑" onclick="edit('${var.ZHOUSE_ID}');">
														<i class="ace-icon fa fa-pencil-square-o bigger-120" title="编辑"></i>
													</a>
													<a class="btn btn-xs btn-success" onclick="addpic('${var.ZHOUSE_ID}');">
														<i class="ace-icon fa fa-folder-open bigger-120" title="添加图片和视频"></i>
													</a>
													</c:if>
													<a class="btn btn-xs btn-success" onclick="view('${var.ZHOUSE_ID}');">
														<i class="ace-icon fa fa-desktop bigger-120" title="预览信息"></i>
													</a>
													<a class="btn btn-xs btn-success" onclick="viewpic('${var.ZHOUSE_ID}');">
														<i class="ace-icon fa fa-camera bigger-120" title="预览图片"></i>
													</a>
													<a class="btn btn-xs btn-success" onclick="viewvideo('${var.ZHOUSE_ID}');">
														<i class="ace-icon fa fa-film bigger-120" title="预览视频"></i>
													</a>
													<c:if test="${userid == var.USERID }">
													<a class="btn btn-xs btn-danger" onclick="del('${var.ZHOUSE_ID}');">
	...
	<script type="text/javascript">
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>zhouse/goAdd.do';
			 diag.Width = 650;
			 diag.Height = 455;
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>zhouse/delete.do?ZHOUSE_ID="+Id+"&tm="+new Date().getTime();
		...
		function addpic(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>pictures/goAdd.do?ZHOUSE_ID='+Id;
			 diag.Width = 800;
			 diag.Height = 490;
			 diag.CancelEvent = function(){ //关闭事件
				 if('${page.currentPage}' == '0'){
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>zhouse/goEdit.do?ZHOUSE_ID='+Id;
			 diag.Width = 650;
			 diag.Height = 455;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function view(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="预览";
			 diag.URL = '<%=basePath%>zhouse/goView.do?ZHOUSE_ID='+Id;
			 diag.Width = 850;
			 diag.Height = 655;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function viewpic(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="预览图片";
			 diag.URL = '<%=basePath%>zhouse/goViewPic.do?ZHOUSE_ID='+Id;
			 diag.Width = 850;
			 diag.Height = 655;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function viewvideo(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="预览视频";
			 diag.URL = '<%=basePath%>zhouse/goViewVideo.do?ZHOUSE_ID='+Id;
			 diag.Width = 850;
			 diag.Height = 655;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>zhouse/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
