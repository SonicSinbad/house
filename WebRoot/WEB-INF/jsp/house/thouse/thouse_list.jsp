...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="mhouse/list.do" method="post" name="Form" id="Form">
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">	
							<thead>
								<tr>
									...
									<th class="center" >卖房统计</th>
									<th class="center" >租房统计</th>
							...
							<c:choose>
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											<td class='center'>${num1 }<c:set var="num1"  value="${num1+ 1}"/> </td>
											<td class='center'>${empty var.asum ? 0 : var.asum}<c:set var="count1"  value="${count1+ var.asum}"/> </td>
											<td class='center'>${empty var.bsum ? 0 : var.bsum}<c:set var="count2"  value="${count2+ var.bsum}"/></td>
											<td class='center'><c:set var="count3"  value="${var.asum+var.bsum}"/>${count3 } </td>
							...
							<tr>
								...
								<td class='center'>${count1+count2 }</td>
								<td class='center'></td>
							</tr>
	...
	<script type="text/javascript">
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>mhouse/goAdd.do';
			 diag.Width = 650;
			 diag.Height = 455;
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>mhouse/delete.do?MHOUSE_ID="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>mhouse/goEdit.do?MHOUSE_ID='+Id;
			 diag.Width = 650;
			 diag.Height = 455;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function view(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="预览";
			 diag.URL = '<%=basePath%>mhouse/goView.do?MHOUSE_ID='+Id;
			 diag.Width = 850;
			 diag.Height = 655;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function viewpic(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="预览图片";
			 diag.URL = '<%=basePath%>mhouse/goViewPic.do?MHOUSE_ID='+Id;
			 diag.Width = 850;
			 diag.Height = 655;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function viewvideo(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="预览视频";
			 diag.URL = '<%=basePath%>mhouse/goViewVideo.do?MHOUSE_ID='+Id;
			 diag.Width = 850;
			 diag.Height = 655;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>mhouse/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
